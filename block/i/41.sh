...
echo "$image"
if [ $i41window -eq 1 ] ; then ./utils.sh overlay win2 12 8 0 ; fi
fi
...
echo "$image"
if [ $i37window -eq 1 ] ; then ./utils.sh overlay win1 26 6 0 ; fi
...
[g][o][n][o][r][t][h]) sed -i '/cell=/c\cell=40' status ; break  ;;

[g][o][s][o][u][t][h]) sed -i '/cell=/c\cell=42' status ; break ;;

[g][o][w][e][s][t]) sed -i '/cell=/c\cell=28' status ; break ;;

[l][o][o][k]) output=$(./utils.sh blockform looki i41 window1 $i41window) ;;

[l][o][o][k][n][o][r][t][h]) output=$(./utils.sh blockform lookdoor corridor4 corridor5 $i40) ;;

[l][o][o][k][s][o][u][t][h]) output=$(./utils.sh blockform lookdoor corridor4 corridor5 $i42) ;;

[l][o][o][k][w][e][s][t]) output=$(./utils.sh blockform lookdoor door1 door19 $i28 1) ;;
...
intro=1
output=$(./utils.sh blockform looki null window15 0 window7 $i1window);;
...
[l][o][o][k]) output=$(./utils.sh blockform looki null window15 0 window7 $i1window);;

[l][o][o][k][r][a][i][n]) output=$(./utils.sh blockform looki null rain4 0) ;;

[l][o][o][k][w][a][l][l]) output=$(./utils.sh blockform looki null wall5 0) ;;

[l][o][o][k][w][i][n][d][o][w]) output=$(./utils.sh blockform looki null window22 0 window7 $i37window) ;;
