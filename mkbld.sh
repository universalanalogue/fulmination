...
output=$(./utils.sh blockform uni back)\
fi ;;\
\
[b][u][r][n][b][o][o][k]) ./utils.sh clear\
if [ $lighter -eq 1 ]\
then\
output=$(./utils.sh blockform burnbooks4)\
else\
...
intro=0 ;;\
\
[l][i][c][k][m][e]) output=$(./utils.sh blockform lickme1) ;;\
\
...
\
[l][o][o][k]*) output=$(./utils.sh blockform uni look) ;;\
\
[l][i][c][k]*) output=$(./utils.sh blockform uni lick1) ;;\
\
...
\
[s][m][e][l][l]*) output=$(./utils.sh blockform uni smell) ;;\
\
...
output=$(./utils.sh blockform installer1 1)
./utils.sh cutscene2 "graphpass i 02pic2b 4 4" Installing "$output"
...
output=$(./utils.sh blockform installer1 2)
./utils.sh cutscene2 "graphpass i 38pic2a 4 4" Installing "$output"
...
output=$(./utils.sh blockform installer1 3)
./utils.sh cutscene2 "graphpass i 05pic2b 4 4" Installing "$output"
...
printf "\e[0;0H"
output=$(./utils.sh blockform installer2)
