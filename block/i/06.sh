...
echo "$image"
if [ $book37 -eq 0 ] ; then ./utils.sh overlay table1 23 10 0 ; fi
fi
...
output=$(./utils.sh blockform uni doorlocked)
fi ;;

[g][o][w][e][s][t]) sed -i '/cell=/c\cell=01' status ; break ;;

[l][o][o][k]) output=$(./utils.sh blockform looki i06) ;;

[l][o][o][k][e][a][s][t]) output=$(./utils.sh blockform lookdoor corridor3 door9 $i08) ;;

[l][o][o][k][n][o][r][t][h]) output=$(./utils.sh blockform lookdoor door1 door5 $i07 $key1) ;;

[l][o][o][k][w][e][s][t]) .output=$(./utils.sh blockform lookdoor corridor4 door3 $i01) ;;
...
[l][o][o][k][d][o][o][r]) output=$(./utils.sh blockform lookdoor door1 door10 $i07 $key1) ;;
...
if [ $book37 -eq 0 ]
then
output=$(./utils.sh blockform looki null hole1 0 hole3 0)
else
output=$(./utils.sh blockform looki null hole2 0)
fi
fi ;;
...
[l][i][c][k][d][o][o][r]) output=$(./utils.sh blockform lick door5) ;;
...
[l][i][c][k][p][e][g]) output=$(./utils.sh blockform lick peg1) ;;

[l][i][c][k][w][a][l][l]) output=$(./utils.sh blockform lick wall) ;;

[s][m][e][l][l]) output=$(./utils.sh blockform smell i06) ;;
...
[s][m][e][l][l][e][a][s][t][c][o][r][r][i][d][o][o][r]) output=$(./utils.sh blockform smell corridor2) ;;

[s][m][e][l][l][w][e][s][t][c][o][r][r][i][d][o][o][r]) output=$(./utils.sh blockform smell corridor2) ;;

[s][m][e][l][l][d][o][o][r]) output=$(./utils.sh blockform smell door4) ;;
...
[l][o][o][k][f][l][o][o][r]) output=$(./utils.sh blockform looki null floor9 0) ;;
