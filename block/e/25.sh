...
echo "$image"
if [ $book16 -eq 0 ] ; then ./utils.sh overlay book8 25 15 0 ; fi
fi
}
...
[g][o][e][a][s][t]) sed -i '/cell=/c\cell=26' status ; break ;;

[g][o][n][o][r][t][h]) sed -i '/cell=/c\cell=32' status ; break ;;

[g][o][w][e][s][t]) sed -i '/cell=/c\cell=24' status ; break ;;

[l][o][o][k]) output=$(./utils.sh blockform elook 25) ;;

[l][o][o][k][e][a][s][t]) output=$(./utils.sh blockform elookdir east $e26 3) ;;

[l][o][o][k][n][o][r][t][h]) output=$(./utils.sh blockform elookdir north $e32 1) ;;

[l][o][o][k][s][o][u][t][h]) output=$(./utils.sh blockform elookdir south 1 4) ;;

[l][o][o][k][w][e][s][t]) output=$(./utils.sh blockform elookdir west $e24 1) ;;
...
intro=1
output=$(./utils.sh blockform elooka foundation1 16 $book16) ;;
...
[l][i][c][k][f][o][u][n][d][a][t][i][o][n]) output=$(./utils.sh blockform lick hole) ;;
...
[s][m][e][l][l][f][o][u][n][d][a][t][i][o][n]) output=$(./utils.sh blockform smell hole) ;;
...
[l][o][o][k]) output=$(./utils.sh blockform elooka foundation1 16 $book16) ;;
...
[l][o][o][k][h][o][l][e]) output=$(./utils.sh blockform elooka foundation1 16 $book16) ;;
