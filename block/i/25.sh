...
output=$(./utils.sh blockform looki null clock1 0)
fi 
fi;;
...
output=$(./utils.sh blockform uni get2 mainspringbarrel)
else
if [ $i25wound -eq 0 ]
then
output=$(./utils.sh blockform uni get3 mainspringbarrel)
else
output=$(./utils.sh blockform looki null clock1 0)
fi 
fi;;
...
output=$(./utils.sh blockform uni get2 sprocket)
else
if [ $i25wound -eq 0 ]
then
output=$(./utils.sh blockform uni get3 sprocket)
else
output=$(./utils.sh blockform looki null clock1 0)
fi 
fi ;;

[g][o][e][a][s][t]) sed -i '/cell=/c\cell=40' status ; break ;;

[l][o][o][k]) output=$(./utils.sh blockform looki i25) ;;
...
then
output=$(./utils.sh blockform looki null clock2 0 clock3 0)
fi
if [ $i25wound -eq 0 ]
then
output=$(./utils.sh blockform looki null clock2 0 clock4 0)
fi
if [ $i25wound -eq 1 ]
then
output=$(./utils.sh blockform looki null clock2 0 clock5 0)
fi ;;
...
output=$(./utils.sh blockform looki null fireplace1 0 fireplace6 $i25wound book10 $book35) ;;
...
then
output=$(./utils.sh blockform looki null clock6 0)
sed -i '/cog=/c\cog=2' status
vars
if [ $cog -eq 2 ] && [ $gear -eq 2 ] && [ $mainspringbarrel -eq 2 ] && [ $sprocket -eq 2 ]
then
output=$(./utils.sh blockform looki null clock7 0)
sed -i '/i25wound=/c\i25wound=0' status
fi
else
output=$(./utils.sh blockform uni place2)
fi ;;
...
then
output=$(./utils.sh blockform looki null clock8 0)
else
output=$(./utils.sh blockform looki null clock6 0)
...
if [ $i25wound -eq 2 ]
then
output=$(./utils.sh blockform looki null clock9 0)
fi
if [ $i25wound -eq 1 ]
then
output=$(./utils.sh blockform looki null clock10 0)
fi 
if [ $i25wound -eq 0 ]
then
...
intro=1
output=$(./utils.sh blockform looki null clock11 0)
fi 
else
output=$(./utils.sh blockform uni wind)
fi ;;
...
[l][i][c][k][w][e][s][t][b][o][o][k][c][a][s][e]) output=$(./utils.sh blockform lick forest) ;;
...
[s][m][e][l][l][w][e][s][t][b][o][o][k][c][a][s][e]) output=$(./utils.sh blockform smell foreste) ;;
